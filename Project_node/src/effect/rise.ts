...
export var rise = (body:HTMLElement) => {
...
uniform sampler2D risemap;

uniform float strength;

void main() {
  vec4 originColor = texture2D(camera, varyUV);
  vec4 texel = texture2D(camera, varyUV);
  vec3 bbTexel = texture2D(blackboard, varyUV).rgb;
  ...
  vec4 mapped;
  mapped.r = texture2D(risemap, vec2(texel.r, .16666)).r;
  mapped.g = texture2D(risemap, vec2(texel.g, .5)).g;
  mapped.b = texture2D(risemap, vec2(texel.b, .83333)).b;
  mapped.a = 1.0;
  ...
  var blackboardLocation = gl.getUniformLocation(program, 'blackboard');
  gl.uniform1i(blackboardLocation, 1);
  var overlayLocation = gl.getUniformLocation(program, 'overlay');
  gl.uniform1i(overlayLocation, 2);
  var risemapLocation = gl.getUniformLocation(program, 'risemap');
  gl.uniform1i(risemapLocation, 3);
  ...
  gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_WRAP_S, gl.CLAMP_TO_EDGE);
  gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_WRAP_T, gl.CLAMP_TO_EDGE);
  gl.texImage2D(gl.TEXTURE_2D, 0, gl.RGBA, gl.RGBA, gl.UNSIGNED_BYTE, document.getElementById('blackboard1024_png') as HTMLImageElement);
  ...
  gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_WRAP_S, gl.CLAMP_TO_EDGE);
  gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_WRAP_T, gl.CLAMP_TO_EDGE);
  gl.texImage2D(gl.TEXTURE_2D, 0, gl.RGBA, gl.RGBA, gl.UNSIGNED_BYTE, document.getElementById('overlaymap_png') as HTMLImageElement);
  ...
  gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_WRAP_S, gl.CLAMP_TO_EDGE);
  gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_WRAP_T, gl.CLAMP_TO_EDGE);
  gl.texImage2D(gl.TEXTURE_2D, 0, gl.RGBA, gl.RGBA, gl.UNSIGNED_BYTE, document.getElementById('risemap_png') as HTMLImageElement);
