...
export var hudson = (body:HTMLElement) => {
...
void main() {
  ...
  vec3 bbTexel = texture2D(background, varyUV).rgb;
  ...
  var backgroundLocation = gl.getUniformLocation(program, 'background');
  gl.uniform1i(backgroundLocation, 1); // texture1に割り当てておく。
  var overlayLocation = gl.getUniformLocation(program, 'overlay');
  gl.uniform1i(overlayLocation, 2); // texture2に割り当てておく。
  var mapLocation = gl.getUniformLocation(program, 'map');
  gl.uniform1i(mapLocation, 3); // texture3に割り当てておく。
  ...
  gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_WRAP_S, gl.CLAMP_TO_EDGE);
  gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_WRAP_T, gl.CLAMP_TO_EDGE);
  gl.texImage2D(gl.TEXTURE_2D, 0, gl.RGBA, gl.RGBA, gl.UNSIGNED_BYTE, document.getElementById('hudsonbackground_png') as HTMLImageElement);
  ...
  gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_WRAP_S, gl.CLAMP_TO_EDGE);
  gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_WRAP_T, gl.CLAMP_TO_EDGE);
  gl.texImage2D(gl.TEXTURE_2D, 0, gl.RGBA, gl.RGBA, gl.UNSIGNED_BYTE, document.getElementById('hudsonmap_png') as HTMLImageElement);
