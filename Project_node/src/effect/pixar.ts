...
export var pixar = (body:HTMLElement) => {
...
vec4 NCTonemapping(vec4 color) {
  vec4 mapped;
  mapped.r = texture2D(pixar, vec2(color.r, 0.0)).r;
  mapped.g = texture2D(pixar, vec2(color.g, 0.0)).g;
  mapped.b = texture2D(pixar, vec2(color.b, 0.0)).b;
  mapped.a = color.a;
  return mapped;
}
...
void main() {
  ...
  // tone mapping
  color.r = texture2D(pixar, vec2(color.r, 0.0)).r;
  color.g = texture2D(pixar, vec2(color.g, 0.0)).g;
  color.b = texture2D(pixar, vec2(color.b, 0.0)).b;
  ...
  var pixarLocation = gl.getUniformLocation(program, 'pixar');
  gl.uniform1i(pixarLocation, 1); // texture1に割り当てておく。
  ...
  gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_WRAP_S, gl.CLAMP_TO_EDGE);
  gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_WRAP_T, gl.CLAMP_TO_EDGE);
  gl.texImage2D(gl.TEXTURE_2D, 0, gl.RGBA, gl.RGBA, gl.UNSIGNED_BYTE, document.getElementById('pixar_curves_png') as HTMLImageElement);
