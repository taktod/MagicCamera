...
export var nashville = (body:HTMLElement) => {
...
uniform sampler2D nashville;

void main() {
  vec3 texel = texture2D(camera, varyUV).rgb;

  texel = vec3(
              texture2D(nashville, vec2(texel.r, .16666)).r,
              texture2D(nashville, vec2(texel.g, .5)).g,
              texture2D(nashville, vec2(texel.b, .83333)).b);
  ...
  var nashvilleLocation = gl.getUniformLocation(program, 'nashville');
  gl.uniform1i(nashvilleLocation, 1); // texture0に割り当てておく。
  ...
  gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_WRAP_S, gl.CLAMP_TO_EDGE);
  gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_WRAP_T, gl.CLAMP_TO_EDGE);
  gl.texImage2D(gl.TEXTURE_2D, 0, gl.RGBA, gl.RGBA, gl.UNSIGNED_BYTE, document.getElementById('nashvillemap_png') as HTMLImageElement);
