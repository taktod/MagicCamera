...
export var exposure = (body:HTMLElement) => {
...
void main() {
  highp vec4 textureColor = texture2D(camera, varyUV);
  ...
  var exposureLocation = gl.getUniformLocation(program, 'exposure');
  gl.uniform1f(exposureLocation, 0.0);
  slider.addEventListener("input", () => {
    gl.uniform1f(exposureLocation, Number(slider.value) / 5.0 - 10.0);
  });
