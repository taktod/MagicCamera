...
export var n1977 = (body:HTMLElement) => {
...
uniform sampler2D n1977;

void main() {
  vec3 texel = texture2D(camera, varyUV).rgb;

  texel = vec3(
              texture2D(n1977, vec2(texel.r, .16666)).r,
              texture2D(n1977, vec2(texel.g, .5)).g,
              texture2D(n1977, vec2(texel.b, .83333)).b);
  ...
  var n1977Location = gl.getUniformLocation(program, 'n1977');
  gl.uniform1i(n1977Location, 1); // texture0に割り当てておく。
  ...
  gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_WRAP_S, gl.CLAMP_TO_EDGE);
  gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_WRAP_T, gl.CLAMP_TO_EDGE);
  gl.texImage2D(gl.TEXTURE_2D, 0, gl.RGBA, gl.RGBA, gl.UNSIGNED_BYTE, document.getElementById('n1977map_png') as HTMLImageElement);
