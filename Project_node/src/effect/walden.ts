...
export var walden = (body:HTMLElement) => {
...
void main()
{
  ...
  texel = vec3(
                texture2D(map, vec2(texel.r, .16666)).r,
  ...
  texel.r = texture2D(vigMap, lookup).r;
  lookup.y = texel.g;
  texel.g = texture2D(vigMap, lookup).g;
  lookup.y = texel.b;
  texel.b	= texture2D(vigMap, lookup).b;
  ...
  var vigMapLocation = gl.getUniformLocation(program, 'vigMap');
  gl.uniform1i(vigMapLocation, 2);
  ...
  gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_WRAP_S, gl.CLAMP_TO_EDGE);
  gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_WRAP_T, gl.CLAMP_TO_EDGE);
  gl.texImage2D(gl.TEXTURE_2D, 0, gl.RGBA, gl.RGBA, gl.UNSIGNED_BYTE, document.getElementById('walden_map_png') as HTMLImageElement);
  ...
  gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_WRAP_S, gl.CLAMP_TO_EDGE);
  gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_WRAP_T, gl.CLAMP_TO_EDGE);
  gl.texImage2D(gl.TEXTURE_2D, 0, gl.RGBA, gl.RGBA, gl.UNSIGNED_BYTE, document.getElementById('vignette_map_png') as HTMLImageElement);
